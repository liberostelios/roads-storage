...
def make_globals(roads_processed, roads, buildings):
  global global_roads_processed
  global_roads_processed = roads_processed
  global global_roads
  global_roads = roads
  global global_buildings
  global_buildings = buildings

def process_road(road_id):
  with print_lock:
    print("Process " + str(os.getppid()) + ": processing road '" + str(road_id) + "' (" + str(global_roads_processed) + "/" + str(len(global_roads)) + ")...")
    global_roads_processed += 1
...
def main():
  ...
  with multiprocessing.Pool(2, initializer=make_globals, initargs=(roads_processed,roads,buildings,)) as pool:
    pool.map(process_road, list(roads)[0:10])
  print("Done!")
